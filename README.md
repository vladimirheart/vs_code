...
- `"printTableBillOnCookingStart": true` — включить поддержку печати заказа от стола: на `CookingStarted` плагин подсказывает ручную печать через кнопку `QR-счет` в экране заказа.
...
Для заказов от стола это особенно важно: `PrintBillCheque(...)` меняет статус заказа на `Bill`, поэтому на живом событии `CookingStarted` заказ часто ещё занят внутренней edit-session iiko и автоматический вызов в рабочих логах упирается в `EntityAlreadyInUseException` до самого закрытия заказа.
Поэтому в текущей версии плагин оставляет для доставки автопечать, а для заказа от стола даёт безопасный ручной сценарий: откройте заказ и нажмите кнопку `QR-счет` в меню плагина на экране заказа.
...
- `"payloadVersion"`:
  ...
  Поле оставлено как служебное для совместимости настроек, но отдельной первой строкой в QR больше не печатается.
...
- `"printTableBillOnCookingStart"`:
  `true` | `false`
  Если `true`, для обычного заказа от стола включается ручная печать через кнопку `QR-счет` на экране заказа.
  Если `false`, плагин не предлагает сценарий печати для заказа от стола.

- `"cookingStartInitialDelayMs"`:
  Любое целое число `>= 0`.
  Значение по умолчанию: `5000`.
  Задержка перед первой попыткой автопечати после события `CookingStarted`.
  Практически влияет только на доставку, потому что для заказа от стола используется ручная кнопка печати.

- `"cookingStartRetryDelayMs"`:
  Любое целое число `> 0`.
  Значение по умолчанию: `2000`.
  Пауза между повторными попытками, если iiko ещё держит блокировку заказа.
  В текущей реализации практически влияет только на доставку.

- `"cookingStartMaxAttempts"`:
  Любое целое число `> 0`.
  Значение по умолчанию: `30`.
  Сколько раз плагин максимум попробует автопечать одного и того же доставочного заказа до окончательного отказа.
...
```text
ЗАК:1542; Д.:2026-07-23T18:42:11; ПОЗ.:2
1;Филадельфия
КОЛ:1; РАЗМЕР:Стандарт
ККАЛ:420; Б:18; Ж:21; У:43
АЛГ:Молоко,Рыба,Соя
МОД.:Доп.сыр[КОЛ:1; ККАЛ:70; Б:4; Ж:6; У:1]
2;Мисо суп
КОЛ:2
ККАЛ:-; Б:-; Ж:-; У:-
АЛГ:-
